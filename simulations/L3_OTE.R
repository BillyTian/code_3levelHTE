...
for (i in 1:nrow(table)){
  ...
  n[i] <- L3_OTE_n_power(eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input)[1]
  pred.power[i] <- L3_OTE_n_power(eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input)[2]
}
...
write.table(result, paste0(mainDir, "/L3_OTE_eff0.2.csv"), sep =",", row.names=F)
