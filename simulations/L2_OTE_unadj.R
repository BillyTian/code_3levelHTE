...
for (i in 1:nrow(table)){
  ...
  n[i] <- L2_OTE2_n(beta3=beta3, beta4=beta4, eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input, rho0=rho0.input, rho1=rho1.input)
  pred.power[i] <- L2_OTE2_power(beta3=beta3, beta4=beta4, n=n[i], eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input, rho0=rho0.input, rho1=rho1.input)
}
...
write.table(result, paste0(mainDir, "/L2_OTE_unadj_eff0.1.csv"), sep =",", row.names=F)
