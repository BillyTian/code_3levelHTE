...
for (i in 1:nrow(table)){
  ...
  n[i] <- L1_HTE_n(eff=delta, m=m.input, p=p.input, alpha0=alpha0.input)
  pred.power[i] <- L1_HTE_power(n=n[i], eff=delta, m=m.input, p=p.input, alpha0=alpha0.input)
}
...
write.table(result, paste0(mainDir, "/L1_HTE_eff0.1.csv"), sep =",", row.names=F)
