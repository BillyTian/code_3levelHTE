...
for (i in 1:nrow(table)){
  ...
  n[i] <- L3_HTE_n(eff=delta, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input, rho0=rho0.input, rho1=rho1.input)
  pred.power[i] <- L3_HTE_power(n=n[i], eff=delta, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input, rho0=rho0.input, rho1=rho1.input)
}
...
write.table(result, paste0(mainDir, "/L3_HTE_eff0.1.csv"), sep =",", row.names=F)
