...
for (i in 1:nrow(table)){
  ...
  n[i] <- L2_OTE_n(eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input)
  pred.power[i] <- L2_OTE_power(n=n[i], eff=delta_OTE, m=m.input, p=p.input, alpha0=alpha0.input, alpha1=alpha1.input)
}
...
write.table(result, paste0(mainDir, "/L2_OTE_eff0.1.csv"), sep =",", row.names=F)
